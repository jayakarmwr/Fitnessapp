        ...
        if person["name"] == name and person["password"] == password:
            ...
            for i in range(len(json_obj)):
                if json_obj[i]["name"] == name:
                    d = json_obj[i]
                    break
            print("Embrack Day", d["day"], "of your fitness journey")
            menu(name)
            break
    else:
...
def calories(name):
    try:
        ...
        if option == 1:
            ...
            d["cal_walking"] = str(int(t * (0.035 * (int(d["weight"]))) + (
                        pow((s * (5 / 18)), 2) / ((int(d["height"])) / 100) * (0.029) * int(d["weight"]))))
